    ...
    card.innerHTML = `
        <div class="item-card__body">
            ...
            <div class="item-card__row">
                ...
                <span class="tag tag--current">${account.badgeNumber}</span>
            </div>
            <div class="item-card__meta">Email: ${account.email}</div>
            <div class="item-card__meta">Department: ${account.department || "—"}</div>
        </div>
    `;
...
async function loadProfile() {
    ...
    clearMessage(msg);
    try {
        const account = await api("/officers/account", { auth: true });
