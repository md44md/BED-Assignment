...
// Send a "new promotion" email to a customer who has ordered from the stall before.
// Same contract as sendQueueNotification: returns { ok, error } and never throws,
// so one failed email can never break the stall owner's create-promotion action.
async function sendPromotionNotification(toEmail, toName, stallName, promotionTitle, promotionDescription) {
    const apiKey = process.env.BREVO_API_KEY;
    const senderEmail = process.env.BREVO_SENDER_EMAIL;
    const senderName = process.env.BREVO_SENDER_NAME || "Hawker Centre Management System";
    ...
    const subject = `New promotion at ${stallName}`;
    const htmlContent = `
        <p>Hi ${toName || "there"},</p>
        <p><strong>${stallName}</strong> has just launched a new promotion:</p>
        <p><strong>${promotionTitle}</strong>${promotionDescription ? `<br />${promotionDescription}` : ""}</p>
        <p>Drop by on your next visit to take advantage of it and save on your order.</p>
        <p>— Hawker Centre Management System</p>
    `;
